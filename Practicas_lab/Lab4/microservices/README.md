...
<!--On Linux (bare-metal execution):
	...
	sudo minikube --vm-driver=none start### 3.1. Creating and deploying your own microservices-based car rental Web API
...
To see the logs related to the Pod:
...
As an example microservices-based application you will create and deploy a simple car rental web API. It will consist in the same functionalities that were described in the lab session about web APIs, so you can **reuse the code and Dockerfile** that you developed in previous web API lab session with these endpoints: 
...
When the carrental service with replicas is deployed in Minikube, try the two endpoints several times, take screenshots of the terminal and interpret the results. 
